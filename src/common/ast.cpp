  ...
  */
  if (_STR_EQ(n->name, "program")) {
    ...
    */
    ...
    if (_STR_EQ(n->children[3]->children[0]->name, "param-list")) {
      ...
      while (param_list_ptr->children_num == 3) {
        s.push(param_list_ptr->children[2]);
        param_list_ptr = param_list_ptr->children[0];
      }
      s.push(param_list_ptr->children[0]);
    ...
    */
    ...
    if (_STR_EQ(n->children[1]->children[0]->name, "local-declarations")) {
      std::stack<syntax_tree_node *> s;
      ...
      while (!s.empty()) {
        auto var_declaration_node =
            static_cast<ASTVarDeclaration *>(transform_node_iter(s.top()));

        auto var_declaration_node_shared = std::shared_ptr<ASTVarDeclaration>(var_declaration_node);
        node->local_declarations.push_back(var_declaration_node_shared);
        s.pop();
      }
    }
    // statement list
    // 2.2 flatten statement-list
    if (_STR_EQ(n->children[2]->children[0]->name, "statement-list")) {
      std::stack<syntax_tree_node *> s;
      ...
      while (!s.empty()) {
        ...
        auto statement_node_shared = std::shared_ptr<ASTStatement>(statement_node);
        node->statement_list.push_back(statement_node_shared);
        s.pop();
      }
    }
    return node;
    ...
    */
    ...
    node->expression = std::shared_ptr<ASTExpression>(expression_node);
    // 5.2 if statement
    auto if_statement_node =
        static_cast<ASTStatement  *>(transform_node_iter(n->children[4]));
    node->if_statement = std::shared_ptr<ASTStatement>(if_statement_node);
    // check whether this selection statement contains
    // 5.3 else structure
    if (n->children_num == 7) {
      auto else_statement_node =
          static_cast<ASTStatement  *>(transform_node_iter(n->children[6]));
      node->else_statement = std::shared_ptr<ASTStatement>(else_statement_node);
    } else {
