...
void Dominators::create_idom(Function *f) {
    bool changed = true;
    // Step 1: Initialize the immediate dominators
    for (auto &bb1 : f->get_basic_blocks()) {
        auto bb = &bb1;
        if (bb == f->get_entry_block()) {
            set_idom(bb, bb);  // The entry block dominates itself
        } else {
            set_idom(bb, nullptr);  // Initially, no basic block dominates others
        }
    }

    // Step 2: Iteratively compute the immediate dominators
    while (changed) {
        changed = false;
        for (auto &bb1 : f->get_basic_blocks()) {
            auto bb = &bb1;
            ...
            // Step 3: Intersect the dominators of the predecessors
            ...
            // Step 4: Update idom if necessary
...
void Dominators::create_dominance_frontier(Function *f) {
    for (auto &bb1 : f->get_basic_blocks()) {
        ...
        for (auto &pred : bb->get_pre_basic_blocks()) {
            ...
            // Step 1: Find the first block in the dominator chain
            while (runner != get_idom(bb)) {
                BBSet &df = dom_frontier_[runner];
                df.insert(bb);  // Add bb to the dominance frontier of runner
                runner = get_idom(runner);  // Move up the dominator chain
            }
        }
    }
}
...
void Dominators::create_dom_tree_succ(Function *f) {
    for (auto &bb1 : f->get_basic_blocks()) {
        ...
        if (idom_bb != nullptr && idom_bb != bb) {
            add_dom_tree_succ_block(idom_bb, bb);  // Add bb as a successor of its immediate dominator
        }
    }
}
